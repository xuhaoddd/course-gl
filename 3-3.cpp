...
void computeTriangleNormals(); //ダミーの関数の宣言

//何分割してアニメーションするか
//課題2-2 : 増やしたり減らしたりしてみる
#define NUM_FRAME 1000

//頂点数と頂点座標の配列 (メモリはファイルを読んだ時に確保済み)
int verN;
float (*ver0)[3]; //t=0 の時の頂点
float (*ver1)[3]; //t=1 の時の頂点
float (*ver)[3]; //モーフィング中の頂点
...
void addTime(){
  ...
  glutPostRedisplay(); // メッシュの再描画を要求する
}
...
int main(int argc, char** argv){
  ...
  normalizeSize(2.0); //メッシュの大きさを視界に合うように拡大縮小する (※ 実寸とは変わる)
  ...
  glutIdleFunc(addTime); //自動で定期的に呼ばれる関数を設定
